...
function diceWinChance() {
  if (diceDirection === "under") return (diceTarget - 1) / 100;
  else                           return (100 - diceTarget) / 100;
}
...
window.rollDice = async function() {
  ...
  if (won) toast(`Gagné ! +${Math.round(bet*mult)} VLX (×${mult.toFixed(2)}) 🎉`, "win");
  else     toast(`Perdu ${bet} VLX — résultat : ${result}`, "lose");
...
function getLastClaim() { return userData?.lastBonus || 0; }
...
window.claimBonus = async function() {
  if (timeUntilNextBonus() > 0) return;
